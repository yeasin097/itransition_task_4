...
SQL_SA_PASSWORD="SqlPassword123!"  # Simple password for intern task (not production)
...
# Step 5: Create the UserManagementDb database
...
# Step 8: Apply EF Core migrations
log "Applying EF Core migrations..."
cd "$APP_DIR"
dotnet ef database update --project "$(dirname "$0")"

# Step 9: Open firewall port for Kestrel
log "Opening firewall port 5000..."
...
log "  cd $PUBLISH_DIR"
log "  dotnet Task4.dll"
log "Then access the app at http://$SERVER_IP:5000"
